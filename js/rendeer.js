...
Renderer.prototype.renderNode = function(node, camera)
{
    ...
    node._uniforms.u_tileGrid = slot;
...
Renderer.prototype.createShaders = function()
{
	...
	this._forward_plus = new GL.Shader('\
		...
		void main() {\
			...
			vec2 uv = vec2( (tileId + 0.5) / float(u_totalTiles), 0.5);\
			vec3 tileData = texture2D(u_tileGrid, uv).xyz;\
			float totalTileLights = tileData.y;\
			float lightIdxOffset = tileData.x;\
			float end = lightIdxOffset + totalTileLights;\
			vec4 textureColor = texture2D(u_color_texture, v_coord);\
			vec3 N = normalize(v_normal);\
			vec3 finalColor;\
			vec3 Iamb = u_ambient * textureColor.xyz;\
			for(float i = 0.0; i < 256.0; i++){\
				if(i >= totalTileLights) break;\
				vec2 uv2 = vec2( float(i + lightIdxOffset + 0.5) / float(u_totalLightIndexes), 0.5);\
				vec3 lightIdx = texture2D(u_tileLightIdTexture, uv2).xyz;\
				vec2 lightUV = vec2( (float(lightIdx.x) + 0.5 ) / float(u_numLights) , 0.5);\
				\
        		vec3 aux = texture2D(u_lightPositionTexture, lightUV).xyz;\
        		vec4 aux2 = texture2D(u_lightColorTexture, lightUV);\
            	vec3 surfaceToLight = normalize(aux - v_position.xyz);\
